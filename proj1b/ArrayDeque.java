public class ArrayDeque<T> implements Deque<T> {
    ...
    private static double SHRINK_FACTOR = 0.5;
    private static int USAGE_THRESHOLD = 4;
    public ArrayDeque(){
        item = (T[])new Object[8];
        size=0;
        nextFirst = 0;
    ...
    * */
    ...
    public T removeFirst(){
        ...
        nextFirst = prevHead;
        size -=1;
        if( item.length > size * USAGE_THRESHOLD) { //reduce the size
            resize(newHead,(int) Math.round(size*SHRINK_FACTOR));
        }
        return rem;
    }
    ...
    public T removeLast(){
        ...
        size -=1;
        if( item.length > size * USAGE_THRESHOLD) { //reduce the size
            resize(stepForward(nextFirst),(int) Math.round(size*SHRINK_FACTOR));
        }
        return rem;
    }
    ...
    public static void main(String[] args){
        LinkedListDeque L2 = new ArrayDeque<>(5);
        L2.addFirst(10);
        L2.addFirst(5);
